...
playlists <- as.list(1:n_weeks)
  
pb <- progress_bar$new(total=n_weeks)
for (k in 610:n_weeks) {
  year = year(date)
  week = week(date)
  ...
  top40 <- as.data.frame(matrix(ncol = 2, nrow = 40))
  colnames(top40) <- c('Song', 'Artist')
  l <- length(listwrapper)
  for(i in 1:l){
    item = listwrapper[[i]]
    song <- html_text(html_node(item, 'p'))
    artist <- html_text(html_node(item, 'h4 a'))
    top40[i,] <- data.frame(song, artist)
  }
  playlists[[k]] <- top40
  write.csv(top40, file = paste('./playlists/', year,'-',week,'.csv', sep = ''))
  date = date + as.period(1,'week')
  pb$tick()
}
saveRDS(playlists, file='listofplaylists.RData')
readRDS('listofplaylists.RData')